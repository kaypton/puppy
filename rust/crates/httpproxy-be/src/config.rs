//! TOML configuration owned by the HTTP proxy backend.

use std::net::ToSocketAddrs;

use serde::Deserialize;

/// Discriminant identifying the HTTP proxy backend in a named configuration
/// group.
pub const TYPE: &str = "httpproxy";

/// TOML configuration for the HTTP proxy backend.
///
...
impl Configuration {
	/// Validates the HTTP proxy backend's own configuration fields.
...
/// delimiter is present. Handles the `proxy_address` shape including IPv6
/// literals (no zone-id handling needed for our tests).
...
mod tests {
	...
	fn split_host_port_simple() {
		assert_eq!(
			split_host_port("proxy.example.com:3128"),
